    ...
    public static string GenLevelDescription(WardenObjectiveType type, WardenObjectiveItem item = WardenObjectiveItem.PersonnelId)
        => type switch
        {
            ...
            WardenObjectiveType.GatherSmallItems => item switch
            {
                ...
                {
                    $"Prisoners to locate encrypted memory stick. Data extraction critical. {IntelCasualtyWarning()}",
                    "Field units ordered to recover lost data storage from hostile sector.",
                    $"Secure memory module detected. Retrieval necessary for Warden systems. {IntelCasualtyWarning()}",
                    "Data fragment isolated in hazardous zone. Recovery team dispatched.",
                    $"Memory device offline. Physical retrieval required. Avoid data corruption. {IntelCasualtyWarning()}",
                    "Intel package stored in portable modules. Must be recovered intact.",
                    $"Prisoners dispatched to acquire sector-specific storage device. {IntelCasualtyWarning()}",
                    "Portable memory carrier located deep in restricted sector. Recover immediately.",
                    $"Encrypted storage critical to system reboot. Retrieve without compromise. {IntelCasualtyWarning()}",
                    "Loss of memory stick will hinder operations. Recovery is mandatory.",
                    $"High-priority retrieval of encrypted drive. Failure not tolerated. {IntelCasualtyWarning()}",
                    "Locate and secure portable drive before hostile sweep.",
                    $"Memory stick data flagged for priority analysis. Prisoners to retrieve now. {IntelCasualtyWarning()}",
                    "Portable module location compromised. Recovery under time pressure.",
                    $"Field recovery of storage device required for Warden objectives. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.PersonnelId => Generator.Pick(new List<string>
                {
                    $"Identity verification incomplete. Prisoners to retrieve personnel ID card. {IntelCasualtyWarning()}",
                    "Personnel clearance tag missing. Retrieve to complete authentication.",
                    $"Sector access locked pending ID retrieval. Locate target credentials. {IntelCasualtyWarning()}",
                    "Locate and secure personnel identification for access chain.",
                    $"Identity record required. Prisoners dispatched to hostile zone to recover. {IntelCasualtyWarning()}",
                    "Lost clearance card located in high-risk sector. Recovery needed.",
                    $"Target identification data essential. Secure without delay. {IntelCasualtyWarning()}",
                    "Personnel ID card flagged in Warden systems. Retrieve intact.",
                    $"Authentication pipeline blocked. Prisoners to recover missing identity token. {IntelCasualtyWarning()}",
                    "ID recovery operation in progress. Maintain possession until extraction.",
                    $"Personnel badge vital for system override. Secure from last known location. {IntelCasualtyWarning()}",
                    "Identity clearance stored in physical card format. Recovery mandated.",
                    $"Locate personnel identification linked to priority subject. {IntelCasualtyWarning()}",
                    "Authorization halted until clearance tag is in Warden possession.",
                    $"Identification artifact retrieval required. Proceed to marked zone. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.PartialDecoder => Generator.Pick(new List<string>
                {
                    $"Decoder fragment required to access restricted protocol. Locate and retrieve. {IntelCasualtyWarning()}",
                    "Partial decryption key located in unstable zone. Recovery required.",
                    $"System unlock incomplete. Missing decoder section must be found. {IntelCasualtyWarning()}",
                    "Decoder segment retrieval necessary for chain command execution.",
                    $"Locate incomplete decoder device to restore access pathway. {IntelCasualtyWarning()}",
                    "Partial decoder module vital for sequence continuation. Secure now.",
                    $"Recovery of decoder piece necessary for uplink. {IntelCasualtyWarning()}",
                    "Decoder shard location verified. Prisoners dispatched.",
                    $"Partial decoder absent from array. Retrieval critical. {IntelCasualtyWarning()}",
                    "Locate decoder section to enable further transmission.",
                    $"System lock bypass blocked until decoder segment retrieved. {IntelCasualtyWarning()}",
                    "Missing decoder element found in high-threat area. Units sent for recovery.",
                    $"Obtain decoder portion for operational continuity. {IntelCasualtyWarning()}",
                    "Decoder fragment to be extracted and integrated immediately.",
                    $"Field units to retrieve decoder section from specified sector. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.Harddrive => Generator.Pick(new List<string>
                {
                    $"Critical data contained on portable hard-drive. Secure and return intact. {IntelCasualtyWarning()}",
                    "Portable disk storage located in hostile environment. Recovery required.",
                    $"Operational continuity dependent on hard-drive retrieval. {IntelCasualtyWarning()}",
                    "Locate large-capacity drive containing key operational files.",
                    $"Field recovery of portable drive essential for system restoration. {IntelCasualtyWarning()}",
                    "Data hub offline until portable storage returned.",
                    $"Drive must be recovered for decryption task. Proceed now. {IntelCasualtyWarning()}",
                    "Locate and secure hard-drive from last known terminal location.",
                    $"Drive houses mission-critical logs. Recovery priority elevated. {IntelCasualtyWarning()}",
                    "Loss of hard-drive unacceptable. Extraction team deployed.",
                    $"Portable disk recovery essential for phase two operations. {IntelCasualtyWarning()}",
                    "Retrieve external drive to continue operations.",
                    $"Secure large data drive before sector lockdown. {IntelCasualtyWarning()}",
                    "Hard-drive retrieval from infected sector authorized.",
                    $"Disk unit loss will terminate mission progress. Retrieve now. {IntelCasualtyWarning()}",
                })!,
                ...
                WardenObjectiveItem.Glp_1 => Generator.Pick(new List<string>
                {
                    ...
                    $"Conduit genetic profile compromised. Recover first-phase GLP sample. {IntelCasualtyWarning()}",
                    "Phase one GLP specimen required for conduit calibration.",
                    $"Retrieve GLP-1 biological sample from marked containment. {IntelCasualtyWarning()}",
                    "Genetic link sample missing. GLP-1 recovery in progress.",
                    $"Field units tasked with collecting GLP-1 target tissue. {IntelCasualtyWarning()}",
                    "Recovery of GLP-1 critical for ongoing conduit research.",
                    $"Locate and secure phase one GLP vial from storage. {IntelCasualtyWarning()}",
                    "Prisoners dispatched to recover GLP-1 from biological vault.",
                    $"Specimen chain incomplete without GLP-1 retrieval. {IntelCasualtyWarning()}",
                    "GLP-1 containment breach suspected. Recover asset.",
                    $"GLP-1 extraction required for final sequence. {IntelCasualtyWarning()}",
                    "Recover GLP-1 tissue for genetic matching process.",
                    $"GLP-1 stored in restricted biolab. Secure immediately. {IntelCasualtyWarning()}",
                    "Locate missing GLP-1 sample before degradation.",
                    $"Biological stability threatened. Retrieve GLP-1 without delay. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.Glp_2 => Generator.Pick(new List<string>
                {
                    "Conduit genetic code compromised. Prisoners to collect DNA sample from HSU facility.",
                    $"Second-phase GLP sample required. Recovery mission active. {IntelCasualtyWarning()}",
                    "GLP-2 biological material essential for conduit link completion.",
                    $"Retrieve GLP-2 specimen from marked secure location. {IntelCasualtyWarning()}",
                    "Field units to recover GLP-2 to finalize chain.",
                    $"Loss of GLP-2 will halt research protocols. {IntelCasualtyWarning()}",
                    "Secure GLP-2 container before hostile breach.",
                    $"GLP-2 recovery authorized for chain integrity. {IntelCasualtyWarning()}",
                    "Prisoners ordered to recover second-stage GLP tissue.",
                    $"Locate GLP-2 vial for conduit completion. {IntelCasualtyWarning()}",
                    "GLP-2 specimen retrieval required for Warden analysis.",
                    $"Biohazard zone contains GLP-2 asset. Retrieve intact. {IntelCasualtyWarning()}",
                    "Transport GLP-2 back to extraction point under guard.",
                    $"GLP-2 loss unacceptable. Secure now. {IntelCasualtyWarning()}",
                    "Find GLP-2 in assigned zone before containment fails.",
                    $"GLP-2 retrieval task initiated. Do not compromise. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.Osip => Generator.Pick(new List<string>
                {
                    $"OSIP unit missing from network chain. Recovery mission engaged. {IntelCasualtyWarning()}",
                    "Portable OSIP module required for operational sync.",
                    $"OSIP hardware offline. Retrieval needed to restore systems. {IntelCasualtyWarning()}",
                    "Locate and recover OSIP device from hostile zone.",
                    $"OSIP data link broken. Recover device to restore feed. {IntelCasualtyWarning()}",
                    "Operational sync impossible without OSIP. Secure now.",
                    $"OSIP hardware asset retrieval authorized. {IntelCasualtyWarning()}",
                    "Field units tasked with retrieving OSIP module.",
                    $"Loss of OSIP breaks uplink protocol. Retrieve immediately. {IntelCasualtyWarning()}",
                    "Locate OSIP module from last known ping coordinates.",
                    $"Recovery of OSIP ensures continuity of system control. {IntelCasualtyWarning()}",
                    "Extract OSIP from compromised storage node.",
                    $"Prisoners deployed to recover OSIP from containment. {IntelCasualtyWarning()}",
                    "Secure OSIP hardware before systems degrade further.",
                    $"Locate OSIP in hostile sector and extract. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.PlantSample => Generator.Pick(new List<string>
                {
                    $"Botanical specimen required for analysis. Retrieve from growth chamber. {IntelCasualtyWarning()}",
                    "Locate plant sample before contamination spreads.",
                    $"Biological sample from flora essential for study. Recover intact. {IntelCasualtyWarning()}",
                    "Hazardous sector contains necessary plant specimen.",
                    $"Plant-based sample needed for environmental scan. {IntelCasualtyWarning()}",
                    "Retrieve botanical specimen from sealed greenhouse.",
                    $"Loss of plant sample will compromise research. {IntelCasualtyWarning()}",
                    "Prisoners deployed to collect flora sample under quarantine.",
                    $"Plant matter required for protocol review. Retrieve carefully. {IntelCasualtyWarning()}",
                    "Specimen located in overgrown zone. Recovery authorized.",
                    $"Botanical recovery required for sequence completion. {IntelCasualtyWarning()}",
                    "Locate and secure plant tissue sample from targeted area.",
                    $"Flora sample extraction to be performed by prisoners. {IntelCasualtyWarning()}",
                    "Overgrowth sector houses required specimen. Recover intact.",
                    $"Botanical sample retrieval critical. Proceed with caution. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.DataCube => Generator.Pick(new List<string>
                {
                    $"Primary data cube detected. Retrieval will restore Warden archives. {IntelCasualtyWarning()}",
                    "Locate cube to recover essential system datasets.",
                    $"Critical cube asset missing. Recovery required for analysis. {IntelCasualtyWarning()}",
                    "Data cube offline. Manual recovery required.",
                    $"Locate and secure primary cube from hostile environment. {IntelCasualtyWarning()}",
                    "Cube houses sensitive archives. Recovery is top priority.",
                    $"Field teams ordered to recover primary data cube. {IntelCasualtyWarning()}",
                    "Cube retrieval operation underway. Maintain security.",
                    $"Loss of cube halts progress. Secure from marked location. {IntelCasualtyWarning()}",
                    "Locate cube to complete primary mission chain.",
                    $"Cube integrity threatened. Recover immediately. {IntelCasualtyWarning()}",
                    "Data cube recovery ensures operational continuity.",
                    $"Cube asset retrieval authorized for current protocol. {IntelCasualtyWarning()}",
                    "Secure cube from hostile-controlled zone.",
                    $"Cube retrieval mandatory for final objective. {IntelCasualtyWarning()}",
                })!,

                WardenObjectiveItem.DataCubeBackup => Generator.Pick(new List<string>
                {
                    $"Backup cube contains secondary system archives. Recovery needed. {IntelCasualtyWarning()}",
                    "Locate backup cube for redundancy protocol.",
                    $"Loss of backup cube will risk data integrity. {IntelCasualtyWarning()}",
                    "Retrieve secondary cube from containment vault.",
                    $"Backup cube needed to restore secondary systems. {IntelCasualtyWarning()}",
                    "Secondary cube location marked. Recover intact.",
                    $"Backup cube retrieval ensures operational failover. {IntelCasualtyWarning()}",
                    "Secure secondary cube to complete redundancy chain.",
                    $"Backup cube offline. Manual extraction required. {IntelCasualtyWarning()}",
                    "Locate and secure secondary cube from hostile area.",
                    $"Cube recovery mission authorized. Backup integrity critical. {IntelCasualtyWarning()}",
                    "Backup cube to be recovered before hostile breach.",
                    $"Prisoners ordered to retrieve redundancy cube. {IntelCasualtyWarning()}",
                    "Backup data cube recovery essential for Warden resilience.",
                    $"Locate cube asset for backup protocol. {IntelCasualtyWarning()}",
                    "Secure backup cube before hostile contact.",
                })!,

                WardenObjectiveItem.DataCubeTampered => Generator.Pick(new List<string>
                {
                    $"Tampered cube detected. Recovery required for forensic analysis. {IntelCasualtyWarning()}",
                    "Locate and secure altered cube for investigation.",
                    $"Compromised cube contains modified data. Retrieve intact. {IntelCasualtyWarning()}",
                    "Retrieve tampered cube before hostile data purge.",
                    $"Data integrity breach suspected. Cube recovery mandated. {IntelCasualtyWarning()}",
                    "Altered data cube may hold critical anomalies. Secure it.",
                    $"Tampered cube recovery ensures evidence preservation. {IntelCasualtyWarning()}",
                    "Field teams deployed to extract compromised data cube.",
                    $"Locate data cube with modified payload for Warden review. {IntelCasualtyWarning()}",
                    "Recovery of tampered cube will aid system security.",
                    $"Cube flagged for tampering. Retrieve without delay. {IntelCasualtyWarning()}",
                    "Compromised cube asset must be secured before analysis.",
                    $"Locate modified cube to continue investigation. {IntelCasualtyWarning()}",
                    "Tampered cube storage location identified. Recovery active.",
                    $"Secure altered cube before data loss escalates. {IntelCasualtyWarning()}",
                })!,
