...
public struct ZoneSensorGroupState
{
    ...
    public uint TriggeredMask;   // Each bit = sensor has been triggered (persistent within session)
    ...
    public ZoneSensorGroupState(bool enabled, uint sensorMask, uint triggeredMask)
    {
        Enabled = enabled;
        SensorMask = sensorMask;
        TriggeredMask = triggeredMask;
    }
    ...
    public ZoneSensorGroupState(ZoneSensorGroupState other)
    {
        ...
        TriggeredMask = other.TriggeredMask;
    }

    public bool IsSensorEnabled(int index) => (SensorMask & (1u << index)) != 0;
    public bool IsSensorTriggered(int index) => (TriggeredMask & (1u << index)) != 0;
